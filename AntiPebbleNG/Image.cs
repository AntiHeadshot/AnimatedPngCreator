...
public abstract class Image(in uint width, in uint height, ColorType colorType, byte bitDepth)
{
    ...
    internal static Image Create(uint width, uint height, byte[] data, byte bitDepth, ColorType colorType)
    {
...
public class Image<TColor>(uint width, uint height, byte[] data) : Image(width, height, typeof(TColor).GetCustomAttribute<ColorAttribute>().ColorType, typeof(TColor).GetCustomAttribute<ColorAttribute>().BitDepth)
